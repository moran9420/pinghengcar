...
int16_t turnout=0;
float gyro_y_speed;
float acc_x_g, acc_y_g, acc_z_g;        // 加速度计数据（单位：g）
float gyro_x_dps, gyro_y_dps, gyro_z_dps; // 陀螺仪数据（单位：°/s）
int16_t angleout = 0;
// 定义度转弧度的系数
...
void TIM1_UP_IRQHandler (void)
{
    if(TIM1->SR & 0x01)
    {
        static uint16_t anglecount = 0;
				static uint16_t count = 0;
		static uint16_t countkey = 0;
        anglecount++;
		 countkey++;
				count++;
        // 1. 蜂鸣器逻辑 (保留，方便排错)
        ...
        {
            beep_cnt--;
            if(beep_cnt == 0) BEEP_OFF();
        }

        // 2. 角度环核心逻辑 (屏蔽速度环、转向环和模式判断)
        // 建议 anglecount 阈值设小一点（比如 > 4），提高直立控制频率
				// TIM1 中断内部修改
				if(anglecount > 4) // 3ms 周期
				{   
						anglecount = 0;
						
						// 1. 获取欧拉角
						Angle_Update(); 
						actualangle = CarAngle+2.8; // 从 Mahony 获取的 Pitch 或 Roll
						
						// 2. 获取真实的角速度 (D项的灵魂)
						// 假设你的车是绕着 Y 轴倒的，就用 mpu6050_gyro_y
						// 必须使用转换后的物理值 (°/s)，不要用原始整数
						 gyro_y_speed = mpu6050_gyro_transition(mpu6050_gyro_y); 
						static float gyro_filt = 0.0f;
					gyro_filt = 0.85f * gyro_filt + 0.15f * gyro_y_speed;
						// 3. 计算输出：传入目标角、实际角、当前角速度
						angleout= pidangcal(targetangle, actualangle, gyro_filt) ;
					motor_set_left(angleout*100);
					motor_set_right(angleout*100);
				...
				{	   
					...
					actualspeed=(encoder_left_speed()+encoder_right_speed())*0.5;
					 targetangle = pidspeedcal(targetspeed*0.01, actualspeed*0.01);
				}
     
        
        // 转向环计算 - 已注释
        // turnout = pidturncal(targetturn, actualturn);
...
//        else
//        {
//            // 左右电机只受角度环 angleout 驱动
//            motor_set_left(angleout);
//            motor_set_right(angleout);
//        }
				
//				if(actualangle>25||actualangle<-25)
//				{
//					motor_set_left(0);
//					motor_set_right(0);						
//				}
 ...
 //  最终版	！！！！			
	...
	//			anglecount=0;
	//			Angle_Update();
	//			actualangle=CarAngle;
	//			//targetspeed=pidangcal(targetangle,actualangle);
	//			angleout=pidangcal(targetangle,actualangle);
	//		}
	//		
	//	/*actualspeed=(encoder_left_speed()+encoder_right_speed())*0.5;
	//	actualturn=encoder_left_speed()-encoder_right_speed();
	//		
	//       if(count>10)
	//	   {count=0;
	//		   key_scanner(); 
	//		     if(path_record_enable)
	//            {	
	//				targetspeed=actualspeed;
	//				targetturn=actualturn;
	//				speedout = 0;   // 不允许 PID 动
	//                turnout  = 0;
	//                path_record_push((int16_t)actualspeed,
	//                                 (int16_t)actualturn);
	//				
	//                
	//            }
	...
	//			Mode3_Switch();
	//		   }  		   
	//		speedout=pidspeedcal(targetspeed,actualspeed);
	//	turnout=pidturncal(targetturn,actualturn);	   
	...
	//    motor_set_left (speedout + turnout * 0.5f);
	//    motor_set_right(speedout - turnout * 0.5f);
	//}
	//	
	//		
	//}*/
...
{
    // 串口接收缓冲中断 (0x00000002 是逐飞库定义的接收中断标志)
    if(UART6->ISR & 0x00000002) 
    {
        uint8_t dat = (uint8_t)UART6->RDR; // 直接读取 RDR 寄存器
        bluetooth_rx_sm(dat);              // 送入状态机
        UART6->ICR |= 0x00000002;          // 清除中断标志位
    }
    
    // 如果有发送中断，直接清除
    if(UART6->ISR & 0x00000001)
    {
        UART6->ICR |= 0x00000001;
    }
}
//-------------------------------------------------------------------------------------------------------------------
